...
Point FixDirection(const Point& p, Point& q, const ScalarField* f) {
  auto dom = f->GetDomain();
  if (! dom.Contains(p)) {
    throw std::domain_error("Domain doesn't contain starting point.");
  }
  double contract_factor = 0.9;
  while (! dom.Contains(p + q)) {
    q = contract_factor * q;
  }
  return q;
}

DirectionFunction::DirectionFunction(const ScalarField* f, const Point& p, Point& q)
  : Function("", Domain {{0, 1}}),
    p_(p),
...
//        std::cout << "WARNING: shrinking argument vector for direction function: x["
//                  << i << "][" << j << "] = " << x[i] << " -> " << dom[i][j] << std::endl;
//        x[i] = dom[i][j];
//      }
//    }
//  }
